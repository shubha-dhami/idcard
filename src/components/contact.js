...
import {BreadcrumbItem,Breadcrumb,Input,Form,FormGroup,Label,Button,Col,FormFeedback} from 'reactstrap';
...
validate(firstname,lastname,telnum,email){
    ...
    if(this.state.touched.firstname && firstname.length < 3)
        errors.firstname='length should be longer';

    if (this.state.touched.lastname && lastname.length < 4)
        errors.lastname = 'length should be longer';

        const reg=/^\d+$/;
    if (this.state.touched.telnum && !reg.test(telnum))
        errors.telnum = 'should be integers';

    if (this.state.touched.email && email.split('').filter(x=> x==='@').length!=1)
        errors.email = 'no @ sign';
